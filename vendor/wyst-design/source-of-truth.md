...
| Proved direct Native typed-operation outcome components with no outcome stack home, plus mandatory materialized fallback across cleanup, aggregate, indirect, explicit-projection, delayed-projection, cross-component, and register-conflict boundaries (`language.typed-operation-direct-native-results`) | Implemented | `wyst.typedOperationKernelComparison.v1` | [Chapter 17](chapter-17-optimization-modes.md); [`typed-operation-kernel-control-budget-v1.json`](typed-operation-kernel-control-budget-v1.json) |
