...
| Object/interface schema bundle | `wyst.objectInterface.v2` | implemented plus future-version rows | Chapter 16 and the semantic database own semantic-module variants, declaration/symbol/member/compatibility identities, generic body/dependency ownership and link-once survivor contracts, the authenticated error-model catalog and outcome-summary slot, object artifact, relocation, and emitted-interface classifications. |
...
| `checked<T>(value)` conversion (`language.checked-conversion`) | Reserved | none | Rejected until the checked outcome implementation milestone activates the selected `wyst.outcome.v1` failure model. |
| One value-success, committed-partial-progress, normal-completion, or typed-error carrier for allocation, parsing, I/O, foreign adapters, boot discovery, container growth, cancellation, deadlines, providers, resource/lifecycle misuse, checked proof/core/synchronization operations, and explicit hardening (`language.error-partial-success-outcome`) | Future-version normative | `wyst.outcome.v1` | Chapter 10; Chapter 15; [`error-model.tsv`](error-model.tsv); `semantic-db.json` `errorModelContract`. The selected contract freezes the declaration, representation, ABI, control flow, partial-progress rule, trap policy, and foreign mapping; the checked outcome implementation milestone activates source use. |
...
| Authenticated error-model catalog, exact four-tag/two-word representation and Native ABI projection, canonical foreign adapter, hardening-trap policy, compatibility-key binding, and outcome-summary interface slot (`artifacts.error-model-contract`) | Implemented contract; source outcome activation remains pending the checked outcome implementation milestone | `wyst.errorModelCatalog.v1` | [`error-model.tsv`](error-model.tsv); Chapters 10, 15, and 16; Appendix A; `semantic-db.json` `errorModelContract` |
...
| `wyst.errorModelCatalog.v1` | Implemented contract | [`error-model.tsv`](error-model.tsv); closed mapping of all selected and downstream failure/progress subjects to `ok`, `partial`, `complete`, `err`, compile-time rejection, or unchanged raw semantics. |
| `wyst.outcome.v1` | Future-version normative | Chapter 10; `core.outcome.Outcome<V, P, E>` with exact tags 0–3, fixed two-word representation, explicit `match` control flow, positive committed partial progress, no implicit propagation/trap/unwind/retry, and source activation deferred to the checked outcome implementation milestone. |
| `wyst.outcomeSummary.v1` | Implemented transport contract | Chapter 16 and Appendix A; digested `WYSTOUT` v1 envelope carrying concrete `V`/`P`/`E` identities, allowed variants, progress unit, and closed trap policy. |
| `wyst.outcomeForeign.v1` | Implemented mapping contract | Chapter 15; explicit `{ tag: u64, payload: u64 }` C bridge with validation and legacy sentinel/status normalization confined to adapters. |
| `wyst.outcomeHardeningTrap.v1` | Implemented policy contract | Chapter 10; an explicit selected terminal adapter may consume `.err(E)`, while ordinary error construction and propagation never trap. |
