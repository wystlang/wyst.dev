...
| PAC not emitted by default                                  | Wyst's current typed-address representation uses untagged 64-bit machine bits plus explicit conversion and named derivation rules. PAC tags occupy top address bits and therefore require an opt-in `#pac` contract; mandatory PAC would require a distinct authenticated address type.                                                                                                 |
